...
module.exports.fileget = (req, res) =>  {
    ...
    result.forEach(function(i) {
        console.log("iioiiiii",i.path);
       // console.log("klvs",result[0].path);
        console.log("jdfksdhfkshf", path.join(__dirname, '../public/'+i.path))
       res.sendFile(path.join(__dirname, '../public/'+i.path));
    }); 
    // let urls=result.map(x=>{
    //     return path.join(__dirname, '../public/'+x.path)
    // });
    // res.send(urls)
    //console.log("klvs",result[0].path);
...
//         console.log("data find of file",result);
//    console.log("image length",result.length)
// let arr = path.join(__dirname, '../public/'+'*.jpeg')
